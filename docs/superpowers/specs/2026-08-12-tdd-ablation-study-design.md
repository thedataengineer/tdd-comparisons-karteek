...
Create three semantically equivalent prompt variants per condition. Variants preserve same obligations, prohibitions, completion criteria, and information while changing wording and order. Freeze and hash variants before first run. Allocate at least two screening repetitions per task to each variant with equal allocation across variants, randomize variant within condition, and include prompt variant plus condition-by-variant interaction in analysis. Confirmation uses at least four paired repetitions per variant with equal allocation. Power analysis may raise either count; equal variant allocation always holds. Material interaction blocks broad condition claim until replication with revised prompt set.
